...
---------------------------------------------------------
### Problem Statement - *Finding out most recently used/visited product in a list of products.*
### Data Structures and Patterns Used - *Doubly Linked Lists, HashMaps*
=========================================================
#### Real Life Scenario
- *A business is interested in knowing the products that has been purchased most recently by a customer. They want to use this information in their promotional and marketing campaigns. They would like to store this information in a suitable data structure where they can easily determine a list of pre-determined number of most recently purchased products. The business is also very critical about speed of the retrieved information from the data structure.*
#### Implementation Detail
- **Assumption** - The important points to note are:
    1. The order of map entries doesn't matter.
    2. The map is used to quickly look if some value is present in the list or not.
    3. The head and tail gives explicit information regarding the start and end of the list, that is, the least recently bought product and the most recently bought product.
    4. When a new item is bought, which is not present in the list, first item (least recently bought product) is removed from the list, and new item is added at the end of the list.
    5. If the item is already present in the list, it is removed from the list and added at the end of the list, without changing the map.
- **Approach** - We will be using doubly linked list, because we want to remove oldest bought product (1st element) if it exceeds the limit of the list (pre-determined number of most recently purchased products). And want to add the most recently bought product at the end of the list. Least recently bought product will be at the start of the list, and most recently bought product will be at the end of the list.
```rust
...
- #### Output
```shell
Product `10` is not in the list. Add it to the tail
After purchasing product `10`   : None <- |10| -> None

Product `15` is not in the list. Add it to the tail
After purchasing product `15`   : None <- |10| <-> |15| -> None

Product `20` is not in the list. Add it to the tail
After purchasing product `20`   : None <- |10| <-> |15| <-> |20| -> None

Product `10` is already in the list. Move it to the tail
Node in question is at the head of list
Move the head to next node and add the removed node to tail
After purchasing product `10`   : None <- |15| <-> |20| <-> |10| -> None

Product `25` is not in the list. Add it to the tail
List is full. Remove the head node and add the new node to the tail
Removing product `15` from the list
After purchasing product `25`   : None <- |20| <-> |10| <-> |25| -> None
